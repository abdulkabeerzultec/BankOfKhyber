...
#End Region
    ...
    Private Function validCompAccess() As Boolean
        If grdViewCompany.RowCount > 0 Then
            For i As Integer = 0 To grdViewCompany.RowCount - 1
                If grdViewCompany.GetRowCellValue(i, "Selection").ToString = True Then
                    Return True
                End If
            Next
        Else
        ...
        Try
            ...
            If valProvMain.Validate Then
                If validCompAccess() Then
                    ...
                    isEdit = True
                Else
                    ShowErrorMessage("Role must have access on at least on Company")
                End If
            End If
    ...
    Private Function AddNewRecord() As Boolean
        Try
            ...
            objattRole.CMAExport = False
            objattRole.CMAImport = chkImportFrmFile.Checked.ToString
            objattRole.DataProcessing = chkDataProcessing.Checked.ToString

            objattRole.AssetsBooks = chkAssetAuditing.Checked
            objattRole.AssetItems = chkInventory.Checked
            objattRole.Units = chkInventoryReports.Checked
            objattRole.Insurer = chkAuditingReports.Checked
            objattRole.Supplier = chkAnonymousRemarks.Checked
            objattRole.PO = chkAnonymousRemarks.Checked
            objattRole.POApproval = chkUpdateExistedAsset.Checked
            objattRole.POTrans = chkDeleteAsset.Checked

            objattRole.OfflineMachine = 0
            objattRole.BackendInventory = 0
            objattRole.Custom1 = 0
            objattRole.Custom2 = 0
            objattRole.Custom3 = 0
            ...
            strCompId = ""
            For i As Integer = 0 To grdViewCompany.RowCount - 1
                If CBool(grdViewCompany.GetRowCellValue(i, "Selection")) <> False Then
                    If strCompId <> "" Then
                        strCompId = strCompId + ","
                    End If
                    strCompId = strCompId + grdViewCompany.GetRowCellValue(i, "CompanyID").ToString
                End If
            Next
    ...
    Private Sub chkCompAstRpt_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            For i As Integer = 0 To grdViewCompany.RowCount - 1
                grdViewCompany.SetRowCellValue(i, "Selection", False)
            Next
    ...
    Private Sub RegisterIntegrationItems()
        chkDepMan.Visible = False
        chkInterCom.Visible = False
        chkAstTrans.Visible = False
        chkDepPolicy.Visible = False
        chkAssetCoding.Visible = False
        chkBarCodePolicy.Visible = False
        chkAstSrch.Visible = False
        chkBarStuct.Visible = False
        chkCreateReport.Visible = False
        'Master Data 


        chkBrand.Visible = False
        chkSupplier.Visible = False
        chkIns.Visible = False
        chkGLCodes.Visible = False
        chkUnits.Visible = False
        chkDisposal.Visible = False
        chkDepre.Visible = False
        chkAssetItem.Visible = False
        chkAstBooks.Visible = False
        chkDept.Visible = False
        TabControl1.TabPages.Remove(tbPO)
    End Sub
    ...
    Private Sub LoadData(ByVal RoleID As String)
        ...
        If RoleID = "1" Then ' if it's admin Role.
            ...
            For idx As Integer = 0 To grdViewCompany.RowCount
                grdViewCompany.SetRowCellValue(idx, "Selection", True)
            Next
            chkAssetAuditing.Checked = True
        ...
        Else
            ...
            If ds Is Nothing = False Then
                If ds.Rows.Count > 0 Then
                    ...
                    chkAssetAuditing.Checked = ds.Rows(0)("AssetsBooks")
                    chkInventory.Checked = ds.Rows(0)("AssetItems")
                    chkInventoryReports.Checked = ds.Rows(0)("Units")
                    chkAuditingReports.Checked = ds.Rows(0)("Insurer")
                    chkAnonymousRemarks.Checked = ds.Rows(0)("Supplier")
                    chkAssetCreation.Checked = ds.Rows(0)("PO")
                    chkUpdateExistedAsset.Checked = ds.Rows(0)("POApproval")
                    chkDeleteAsset.Checked = ds.Rows(0)("POTrans")
                    ...
                    If ds.Rows(0)("Companies").ToString <> "" Then
                        ...
                        If strCompIds.Length > 0 Then
                            ...
                            For i As Integer = 0 To strCompIds.Length - 1
                                RowHandle = grdViewCompany.LocateByDisplayText(0, grdViewCompany.Columns("CompanyID"), strCompIds(i))
                                If RowHandle >= 0 Then
                                    grdViewCompany.SetRowCellValue(RowHandle, "Selection", True)
                                Else
                                    grdViewCompany.SetRowCellValue(RowHandle, "Selection", False)
                                End If
                            Next
                        End If
                    End If
    ...
    Private Sub btnNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNew.Click
        ...
        btnSave.Enabled = True
        txtRoleDesc.Focus()
        chkAssetAuditing.Checked = False
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        Try
            If Not check_Child_Roles(lovRoleID.SelectedText, 1) Then
                If MessageBox.Show("Do you really want to delete this record ?", "ZulAssets", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
                    objattRole = New attRoles
                    objattRole.PKeyCode = lovRoleID.SelectedText
                ...
                End If
            Else
                ShowErrorMessage("This record cannot be deleted as related records exists.")
            End If
    ...
    Private Sub chkAll_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkAll.CheckedChanged
        For i As Integer = 0 To grdViewCompany.RowCount - 1
            grdViewCompany.SetRowCellValue(i, "Selection", chkAll.Checked)
        Next
    End Sub
    ...
    Private Sub chkRptAll_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkRptAll.CheckedChanged
        ...
        chkAuditingReports.Checked = chkRptAll.Checked
        chkInventoryReports.Checked = chkRptAll.Checked
    End Sub
End Class
