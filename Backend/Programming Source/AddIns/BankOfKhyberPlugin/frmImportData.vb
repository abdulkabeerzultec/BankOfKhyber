...
Public Class frmImportData
    ...
    Private Function CheckColumnsMasterData(ByVal dt As DataTable) As Boolean
        ...
        '    Result = False
        'End If
        If dt.Columns("ASSET Supplier") Is Nothing Then
            Result = False
        End If
        ...
        Try
            ...
            For i As Integer = 0 To grdView.RowCount - 1
                ...
                If Not String.IsNullOrEmpty(grdView.GetRowCellValue(i, ("Main Location")).ToString.Trim) Then
                    ...
                    LocID = ImportLocation(grdView.GetRowCellValue(i, ("Main Location")).ToString.Trim, grdView.GetRowCellValue(i, ("LOCATION")).ToString.Trim)
                End If
                'Import Employee
                ...
                If Not String.IsNullOrEmpty(grdView.GetRowCellValue(i, ("Custodian")).ToString.Trim.Replace("'", "")) Then
                    EmpID = ImportEmp(grdView.GetRowCellValue(i, ("Custodian")).ToString.Trim.Replace("'", ""), "")
                ...
                If Not String.IsNullOrEmpty(grdView.GetRowCellValue(i, ("Asset Supplier")).ToString.Trim) Then
                    If grdView.GetRowCellValue(i, ("Asset Supplier")).ToString.Trim.Length > 50 Then
                        SupplierID = ImportSupplier(grdView.GetRowCellValue(i, ("Asset Supplier")).ToString.Trim.Substring(0, 50).Replace("'", ""))
                    Else
                        SupplierID = ImportSupplier(grdView.GetRowCellValue(i, ("Asset Supplier")).ToString.Trim)
                ...
                If Not String.IsNullOrEmpty(grdView.GetRowCellValue(i, ("SYSTEM ASSET ID")).ToString.Trim) Then
                    Barcode = grdView.GetRowCellValue(i, ("SYSTEM ASSET ID")).ToString.Trim
                End If
                If ImportAsset(focusrowhandel, CatID, LocID, EmpID, CompanyID, AstBrandID, dtAssets, dtAssetsDetails, dtInvSch, GLCode, SupplierID, Barcode) Then
                    importedcount += 1
                End If
    ...
    Private Function ImportLocation(ByVal Location1 As String, ByVal Location2 As String)
        ...
        RoomID = objBALLocation.GetLocIDByCompleteDesc(Location1 & " \ " & Location2)
    ...
    Private Function ImportAsset(ByVal FocuseRowHandel As Integer, ByVal CatID As String, ByVal LocationID As String, ByVal CustodianID As String, ByVal CompanyID As String, ByVal AstBrandID As String, ByRef dtAssets As DataTable, ByVal dtAssetDetails As DataTable, ByVal dtInvSch As DataTable, ByVal GLCode As String, ByVal Supplier As Integer, ByVal Barcode As String) As Boolean
        'Add Asset Item
        Try
            Dim ItemDesc As String = grdView.GetRowCellValue(FocuseRowHandel, ("Asset Description")).ToString.Trim
            Dim AssetRefNumber As String
            '= grdView.GetRowCellValue(FocuseRowHandel, ("Serial #")).ToString.Trim
            Dim AssetItemID As String = String.Empty

            Dim objattAssetDetails As New attAssetDetails
            Dim objBALAssetDetails As New BALAssetDetails

            Dim ser As String = objBALAssetDetails.GetAssetsCount(objattAssetDetails)
            Dim AssetID As String = objBALAssetDetails.Generate_AssetID()
            AssetRefNumber = ser + 1
            AssetItemID = String.Empty
    ...
    Private Function ImportCategory(ByVal MainCat As String, ByVal SalvageYear As Double, ByVal SalvageValue As Double) As String
