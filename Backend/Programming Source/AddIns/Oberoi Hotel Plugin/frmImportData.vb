...
Public Class frmImportData
    ...
    Private Function CheckColumnsMasterData(ByVal dt As DataTable) As Boolean
        ...
        If dt.Columns("Barcode ID") Is Nothing Then
            Result = False
        End If

        If dt.Columns("Main Category") Is Nothing Then
            Result = False
        End If
        ...
        If dt.Columns("Asset Category") Is Nothing Then
            Result = False
        End If
        If dt.Columns("Asset Sub Category") Is Nothing Then
            Result = False
        End If
     
        If dt.Columns("Main Location") Is Nothing Then
            Result = False
        End If

        If dt.Columns("Area") Is Nothing Then
            Result = False
        End If
        If dt.Columns("Level") Is Nothing Then
            Result = False
        End If
        If dt.Columns("Section") Is Nothing Then
            Result = False
        End If
        If dt.Columns("Room No") Is Nothing Then
            Result = False
        End If
        If dt.Columns("Value") Is Nothing Then
            Result = False
        End If
    ...
    Private Function ImportAssetCreation() As String
        ...
        Try
            'Import Company
            Dim CompanyID As String = ImportCompany("Oberoi Hotel")
            Dim EmpID As String = ImportEmp("Unknown", "")
            For i As Integer = 0 To grdView.RowCount - 1

                Dim CatID As String = 1
                If Not String.IsNullOrEmpty(grdView.GetRowCellValue(i, ("Main Category")).ToString.Trim) Then
                    CatID = ImportCategory(grdView.GetRowCellValue(i, ("Main Category")).ToString.Trim, grdView.GetRowCellValue(i, ("Asset Category")).ToString.Trim)
                End If
                'Import location.
                ...
                If Not String.IsNullOrEmpty(grdView.GetRowCellValue(i, ("Main Location")).ToString.Trim) Then
                    ...
                    loc.City = grdView.GetRowCellValue(i, ("Main Location")).ToString.Trim
                    loc.Building = grdView.GetRowCellValue(i, ("Area")).ToString.Trim
                    loc.Floor = grdView.GetRowCellValue(i, ("Level")).ToString.Trim
                    loc.Room = grdView.GetRowCellValue(i, ("Section")).ToString.Trim
                    loc.Station = grdView.GetRowCellValue(i, ("Room No")).ToString.Trim
    ...
    Private Function ProcessAssetCreationFile() As Boolean
        ...
        If CheckColumnsMasterData(Sheetdt) Then
            ...
            grdView.Columns("Value").DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric
            grdView.Columns("Value").DisplayFormat.FormatString = "#.##"
    ...
    Private Function ImportAsset(ByVal FocuseRowHandel As Integer, ByVal CatID As String, ByVal LocationID As String, ByVal CustodianID As String, ByVal CostCenterID As String, ByVal CompanyID As String, ByVal AstBrandID As String, ByRef dtAssets As DataTable, ByVal dtAssetDetails As DataTable, ByVal dtInvSch As DataTable, ByVal GLCode As String) As Boolean
        'Add Asset Item
        Try
            Dim ItemDesc As String = grdView.GetRowCellValue(FocuseRowHandel, ("Asset Sub Category")).ToString.Trim
            Dim AssetRefNumber As String = grdView.GetRowCellValue(FocuseRowHandel, ("Barcode ID")).ToString.Trim
            ...
            objattAssetDetails.BaseCost = grdView.GetRowCellDisplayText(FocuseRowHandel, ("Value")).ToString.Trim
            ...
            objattAssetDetails.RefNo = AssetRefNumber.Remove(0, 3).Trim

            Dim AstNum As Int64 = 0
            'Extract Asset number from the barcode field: TOD015702

            If Int64.TryParse(AssetRefNumber.Remove(0, 3).Trim, AstNum) Then
            ...
            Else
                ...
                If objBALAssetDetails.Insert_AssetDetails(objattAssetDetails, False) Then
                    objBALAssetDetails.Add_AstBooks(objattAssetDetails.PKeyCode, grdView.GetRowCellDisplayText(FocuseRowHandel, ("Value")).ToString.Trim, objattAssetDetails.CompanyID, 1, UsefulLife, 0, objattAssetDetails.SrvDate, False)
