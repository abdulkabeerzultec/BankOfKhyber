...
Module DepCalc
    ...
    '            Dim fiscalYr As Date = ServiceDate.AddMonths(TotalSalMon)
    ...
    '            Dim startOfMonth As DateTime = ServiceDate.AddDays(1)
    '            Dim endOfMonth As DateTime = GetLastDayInMonth(ServiceDate).AddDays(1)

    '            If MnthLeft > 0 Then
    '                For i As Integer = 0 To MnthLeft - 1
    '                    depValue = CalcDep(SalYr, SalMon, intDeptype, Price, SalValue, endOfMonth, startOfMonth)
    ...
    '                    Else
    '                        accDepValue += depValue
    '                        dt.Rows.Add(startOfMonth, CDbl(CurBookValue - accDepValue + depValue), CDbl(CurBookValue - accDepValue + depValue), CurBookValue, SalValue)
    '                        Exit For
    '                    End If
    ...
    'Public Function CalcDepAnnual(ByVal BookID As String, ByVal SalYr As Int16, ByVal SalMon As Int16, ByVal intDeptype As Integer, ByVal Price As Double, ByVal SalValue As Double, ByVal ServiceDate1 As String, ByVal CurBookValue As Double, ByVal dtService As Date) As DataTable
    '    Try
    '        If SalYr > 0 Then
    ...
    '            End Try
    '            ServiceDate = GetLastDayInMonth(ServiceDate)
    ...
    '            Dim preCounter As Date = ServiceDate
    '            Dim dtCounter As Date = ServiceDate.AddYears(1)

    '            If MnthLeft > 0 Then
    '                For i As Integer = 0 To MnthLeft - 1

    '                    dtCounter = GetLastDayInMonth(dtCounter)
    '                    depValue = CalcDep(SalYr, SalMon, intDeptype, Price, SalValue, dtCounter, preCounter)
    '                    If CDbl(CurBookValue - accDepValue - depValue) >= SalValue Then
    '                        accDepValue += depValue
    ...
    '                    Else
    '                        If CurBookValue - accDepValue = 0 Then
    '                            Exit For
    '                        Else
    '                            accDepValue += depValue
    '                            dt.Rows.Add(preCounter, CDbl(CurBookValue - accDepValue + depValue), CDbl(CurBookValue - accDepValue + depValue), CurBookValue, SalValue)
    '                            Exit For
    '                        End If
    '                    End If
